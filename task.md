# Task List

## Day 1: Foundation - Linux Primitives
- [x] Check for Linux Environment (WSL) <!-- id: 0 -->
- [x] Task 1.1: Create Network Namespaces <!-- id: 1 -->
- [x] Task 1.2: Build a Virtual Bridge Network <!-- id: 2 -->
- [x] Task 1.3: Implement NAT for Internet Access <!-- id: 3 -->
- [x] Task 1.4: Setup Port Forwarding <!-- id: 4 -->

## Day 2: Application Services
- [x] Task 2.1: Deploy Nginx Load Balancer <!-- id: 5 -->
- [x] Task 2.2: Create API Gateway <!-- id: 6 -->
- [x] Task 2.3: Build Product Service <!-- id: 7 -->
- [x] Task 2.4: Build Order Service <!-- id: 8 -->
- [x] Task 2.5: Deploy Redis and PostgreSQL <!-- id: 9 -->

## Day 3: Monitoring and Debugging
- [ ] Task 3.1: Network Traffic Analysis <!-- id: 10 -->
- [ ] Task 3.2: Service Health Monitoring <!-- id: 11 -->
- [ ] Task 3.3: Connection Tracking Analysis <!-- id: 12 -->
- [ ] Task 3.4: Create Network Topology Visualizer <!-- id: 13 -->

## Day 4: Advanced Networking
- [x] Task 4.1: Implement Simple Service Discovery <!-- id: 14 -->
- [x] Task 4.2: Implement Round-Robin Load Balancing <!-- id: 15 -->
- [ ] Task 4.3: Network Security Policies <!-- id: 16 -->
- [x] Task 4.4: Implement Network Isolation <!-- id: 17 -->

## Day 5: Docker Migration
- [x] Task 5.1: Containerize All Services <!-- id: 18 -->
- [x] Task 5.2: Docker Compose Setup <!-- id: 19 -->
- [x] Task 5.3: Performance Comparison <!-- id: 20 -->
- [x] Task 5.4: Optimize Docker Setup <!-- id: 21 -->

## Day 6: Multi-Host Networking (Optional)
- [ ] Task 6.1: Setup VXLAN Overlay <!-- id: 22 -->
- [ ] Task 6.2: Docker Swarm Setup <!-- id: 23 -->

## Day 7: Documentation
- [x] Task 7.1: Technical Documentation <!-- id: 24 -->
- [ ] Task 7.2: Create Presentation <!-- id: 25 -->
